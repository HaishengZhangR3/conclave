...
public class Host {
    public static void main(String[] args) throws EnclaveLoadException, IOException {
        ...
        try (EnclaveHost enclave = EnclaveHost.load("com.r3.conclave.sample.enclave.DiabetesEnclave")) {
            ...
            try {
                ...
                // >>>>>>>>>>>>>>> 2nd data for training >>>>>>>>>>>>>>>>>>>>>>>>
                {
                    ...
                    // Deliver it. The enclave will give us some mail to reply with via the callback we passed in
                    // to the start() method.
                    enclave.deliverMail(2, mailBytes);
                    byte[] toSend = requestToDeliver.get();
                    output.writeInt(toSend.length);
                    output.write(toSend);
                }
                ...
                for (int i = 0; i < recordNum; i++){
                    ...
                    // Deliver it. The enclave will give us some mail to reply with via the callback we passed in
                    // to the start() method.
                    enclave.deliverMail(i+10, mailBytes);
                    byte[] toSend = requestToDeliver.get();
                    output.writeInt(toSend.length);
                    output.write(toSend);
                }

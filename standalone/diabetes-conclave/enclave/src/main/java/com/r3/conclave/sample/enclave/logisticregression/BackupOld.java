...
public class BackupOld {
    ...
    public static String getResults(String filename, int restarts, int foldsNum, int epochs, double learningRate, int numOfWeights) throws FileNotFoundException {
        ...
        for (int q = 0; q < restarts + 1; q++) {//Run for certain number of restarts
            BackupOld logistic = new BackupOld(numOfWeights);
    ...
    public static void main(String... args) throws IOException {
         ...
         * */
        final String path = "/Users/haishengzhang/git/_conclave/ml/diabetes-ml/src/main/java/";
        ...
        for (int i = 0; i < 1000000; i++) {
            ...
            dataset = path + "D-[Extracted Features].txt";
            //dataset = "C-[Replaced by Mean].txt";
            //dataset = "A-[Unprocessed].txt";
            if (dataset.equals(path + "D-[Extracted Features].txt")) result = getResults(dataset, 5, 5, 300, 0.15, 5);
            else result = getResults(dataset, 5, 5, 300, 0.15, 8);
            System.out.println(result);
            double threshold = 0;
            if (dataset.equals(path + "B-[Excl Missing].txt")) threshold = 83.5;
            else if (dataset.equals(path + "D-[Extracted Features].txt")) threshold = 80.5;
            else if (dataset.equals(path + "C-[Replaced by Mean].txt")) threshold = 79.5;
            else threshold = 63.5;
